...
# ==============================================================================
elif menu == "🎨 Protótipo Cards":
    st.title("🎨 Protótipo - Layout em Cards")
    st.caption("Visualização experimental de pedidos em formato de cards (similar a dashboards modernos)")
    ...
    with col_filtro1:
        data_filtro = st.selectbox(
            "📅 Data",
            ["Hoje", "Esta Semana", "Este Mês", "Todos"],
            index=0
        )
    ...
    with col_filtro2:
        status_filtro = st.multiselect(
            ...
            default=["🔴 Pendente", "🟡 Em Produção", "✅ Entregue"]
        )
    ...
    with col_filtro3:
        cards_por_linha = st.selectbox(
            "Cards/linha",
            [2, 3, 4, 5],
            index=2  # 4 cards por linha (padrão)
        )

    # Aplicar filtros de data
    if data_filtro == "Hoje":
        df_filtrado = df[df['Data'] == hoje_brasil()]
    elif data_filtro == "Esta Semana":
        inicio_semana = hoje_brasil() - timedelta(days=hoje_brasil().weekday())
        df_filtrado = df[df['Data'] >= inicio_semana]
    elif data_filtro == "Este Mês":
        df_filtrado = df[df['Data'].apply(lambda x: x.month == hoje_brasil().month and x.year == hoje_brasil().year)]
    else:
        df_filtrado = df
    ...
    df_filtrado = df_filtrado.sort_values('ID_Pedido', ascending=False)
    ...
    with c2:
        caruru_total = df_filtrado['Caruru'].sum()
        st.metric("🥘 Total Caruru", f"{int(caruru_total)} kg")

    with c3:
        bobo_total = df_filtrado['Bobo'].sum()
        st.metric("🦐 Total Bobó", f"{int(bobo_total)} kg")

    with c4:
        # Não cancelados
        df_nao_cancelados = df_filtrado[~df_filtrado['Status'].str.contains("Cancelado", na=False)]
        valor_total = df_nao_cancelados['Valor'].sum()
        st.metric("💰 Faturamento", formatar_valor_br(valor_total))
    ...
    if df_filtrado.empty:
        ...
        st.subheader(f"📋 {len(df_filtrado)} Pedidos Encontrados")

        # Renderizar cards em grid
        pedidos_lista = df_filtrado.to_dict('records')

        for i in range(0, len(pedidos_lista), cards_por_linha):
            cols = st.columns(cards_por_linha)

            for idx, col in enumerate(cols):
                if i + idx < len(pedidos_lista):
                    pedido = pedidos_lista[i + idx]
                    ...
                    with col:
                        ...
                        with col_btn1:
                            if st.button("👁️ Ver", key=f"view_card_{pedido['ID_Pedido']}", use_container_width=True):
                                st.session_state[f"expandir_card_{pedido['ID_Pedido']}"] = not st.session_state.get(f"expandir_card_{pedido['ID_Pedido']}", False)

                        with col_btn2:
                            if st.button("✏️ Editar", key=f"edit_card_{pedido['ID_Pedido']}", use_container_width=True):
                                st.session_state[f"editar_card_{pedido['ID_Pedido']}"] = not st.session_state.get(f"editar_card_{pedido['ID_Pedido']}", False)

                        # Expansor para ver detalhes
                        if st.session_state.get(f"expandir_card_{pedido['ID_Pedido']}", False):
                            with st.expander("📋 Detalhes Completos", expanded=True):
                                st.write(f"**ID:** {int(pedido['ID_Pedido'])}")
                                st.write(f"**Cliente:** {pedido['Cliente']}")
                                st.write(f"**Contato:** {pedido['Contato']}")
                                st.write(f"**Data:** {pedido['Data'].strftime('%d/%m/%Y')}")
                                st.write(f"**Hora:** {pedido['Hora'].strftime('%H:%M') if isinstance(pedido['Hora'], time) else str(pedido['Hora'])[:5]}")
                                st.write(f"**Caruru:** {int(pedido['Caruru'])} kg")
                                st.write(f"**Bobó:** {int(pedido['Bobo'])} kg")
                                st.write(f"**Desconto:** {pedido['Desconto']}%")
                                st.write(f"**Valor:** {formatar_valor_br(pedido['Valor'])}")
                                st.write(f"**Status:** {pedido['Status']}")
                                st.write(f"**Pagamento:** {pedido['Pagamento']}")
                                if pedido.get('Observacoes') and str(pedido['Observacoes']).strip() and str(pedido['Observacoes']).strip() != "nan":
                                    st.write(f"**Observações:** {pedido['Observacoes']}")

                        # Formulário de edição (placeholder)
                        if st.session_state.get(f"editar_card_{pedido['ID_Pedido']}", False):
                            with st.expander("✏️ Editar Pedido", expanded=True):
                                st.info("💡 Funcionalidade de edição será integrada aqui (mesmo que a aba 'Gerenciar Tudo')")
                                if st.button("❌ Fechar", key=f"fechar_edit_card_{pedido['ID_Pedido']}"):
                                    st.session_state[f"editar_card_{pedido['ID_Pedido']}"] = False
                                    st.rerun()
    ...
    with st.expander("ℹ️ Sobre este Protótipo"):
        st.markdown("""
        ### 🎨 Layout em Cards - Protótipo Experimental

        **Características:**
        - ✅ Visualização em grid responsivo (2-5 cards por linha)
        - ✅ Cores de borda baseadas no status do pedido
        ...
        **Vantagens:**
        - 📊 Melhor escaneamento visual
        - 🎯 Identificação rápida por cores
        - 💡 Aproveitamento de espaço horizontal
        - 🎨 Interface mais moderna e profissional

        **Diferenças do layout atual:**
        - Cards ocupam mais espaço vertical
        - Melhor para visualização rápida de status
        - Menos denso em informações por tela
        - Mais adequado para operação diária

        **💡 Sugestão:** Use este layout para "Pedidos do Dia" e mantenha o layout tabular para "Gerenciar Tudo" (análise detalhada).
        """)

    st.caption("🔬 Este é um protótipo experimental. Seu feedback é importante!")
