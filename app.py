...
import urllib.parse
import re
import shutil # Para backup seguro
...
# ==============================================================================
# FUNÇÕES DE VALIDAÇÃO E SEGURANÇA (NOVAS)
# ==============================================================================
...
def validar_telefone(telefone):
    """Valida formato DDD + Número (10 ou 11 dígitos)."""
    limpo = limpar_telefone(telefone)
    if not limpo: return "", None # Vazio é permitido, mas retorna vazio
    
    # Remove 55 se o usuário colocou
    if limpo.startswith("55") and len(limpo) > 11:
        limpo = limpo[2:]
        
    if len(limpo) < 10 or len(limpo) > 11:
        return limpo, f"⚠️ Telefone estranho ({len(limpo)} dígitos). Verifique o DDD."
    
    return limpo, None

def cliente_existe(nome, df_clientes):
    """Verifica duplicidade de nome (case insensitive)."""
    if df_clientes.empty: return False
    nomes_existentes = df_clientes['Nome'].astype(str).str.strip().str.lower()
    return nome.strip().lower() in nomes_existentes.values
...
def calcular_total(caruru, bobo, desconto):
    try:
        ...
        if d > 0:
            total = total * (1 - d / 100.0)
        return round(total, 2)
    except: return 0.0

# ==============================================================================
# BANCO DE DADOS (COM SINCRONIZAÇÃO)
# ==============================================================================
...
def salvar_pedidos(df):
    try:
        ...
        if os.path.exists(ARQUIVO_PEDIDOS):
            shutil.copy(ARQUIVO_PEDIDOS, ARQUIVO_PEDIDOS + ".bak")
...
def registrar_alteracao(tipo, id_pedido, campo, valor_antigo, valor_novo):
    try:
        ...
        if len(df) > 500: df = df.tail(500) # Limita tamanho
...
if menu == "Dashboard do Dia":
    ...
    # Sincroniza dados para garantir que está vendo o mais recente
    ...
    if df.empty: st.info("Sem dados.")
        ...
        agora = datetime.now()
        df_hoje = df[(df['Data'] == date.today()) & (~df['Status'].str.contains("Entregue|Cancelado", na=False))].copy()
        ...
        pend = df_dia[(~df_dia['Status'].str.contains("Entregue", na=False)) & (~df_dia['Status'].str.contains("Cancelado", na=False))]
        ...
        for _, row in df_hoje.iterrows():
            if row['Hora']:
                dt_ent = datetime.combine(date.today(), row['Hora'])
                diff = (dt_ent - agora).total_seconds() / 60
                if 0 < diff <= 120:
                    st.warning(f"⏰ ALERTA: Pedido de **{row['Cliente']}** para {row['Hora'].strftime('%H:%M')} (Faltam {int(diff)} min)")
        ...
        if not df_dia.empty:
            ...
            if not edited.equals(df_dia):
                df_glob = carregar_pedidos() # Recarrega do disco para evitar sobrescrita
                for i in edited.index:
                    idp = edited.at[i, 'ID_Pedido']
                    mask = df_glob['ID_Pedido'] == idp
                    if mask.any():
                        # Registra alteração de status
                        old_st = df_glob.loc[mask, 'Status'].values[0]
                        new_st = edited.loc[i, 'Status']
                        if old_st != new_st:
                            registrar_alteracao("STATUS_DASH", idp, "Status", old_st, new_st)
        ...
        st.subheader("📈 Vendas (Últimos 7 Dias)")
        inicio = date.today() - timedelta(days=7)
        df_chart = df[df['Data'] >= inicio].copy()
        if not df_chart.empty:
            vendas = df_chart.groupby('Data')['Valor'].sum()
            st.bar_chart(vendas)
    ...
    with st.form("form_novo", clear_on_submit=True):
        ...
        if st.form_submit_button("💾 SALVAR"):
            ...
            erro = False
            if not c_sel: 
                st.error("❌ Selecione um cliente."); erro = True
            if qc <= 0 and qb <= 0:
                st.error("❌ O pedido deve ter pelo menos 1 item."); erro = True
            if qc < 0 or qb < 0:
                st.error("❌ Quantidade negativa não permitida."); erro = True
            ...
            if not erro:
                try:
                    ...
                    registrar_alteracao("CRIAR", nid, "Novo Pedido", "", f"{c_sel} | R$ {val}")
    ...
    # Recarrega sempre para garantir visão atual
    st.session_state.pedidos = carregar_pedidos()
    df = st.session_state.pedidos
    ...
    if not df.empty:
        ...
        if not edited.equals(df):
            try:
                ...
                # Registra log genérico de edição em massa
                registrar_alteracao("EDIT_MASSA", 0, "Tabela", "", "Alteração na tabela geral")
        ...
        with c1:
            ...
            try:
                ...
                if sel:
                    d = df[df['Cliente'] == sel].iloc[0] # Pega o mais recente
                    t = limpar_telefone(d.get('Contato'))
                    ...
                    else: st.warning("Telefone inválido.")
            except: pass
        with c2:
             ...
             with st.expander("Abrir Exclusão"):
                 ...
                 if st.button("CONFIRMAR EXCLUSÃO"):
                     if ped_del:
                         ...
                         salvar_pedidos(st.session_state.pedidos)
                         registrar_alteracao("EXCLUIR", id_apagar, "Pedido", "", "Excluído")
    ...
    with c_img:
        up_img = st.file_uploader("Banner (Visual)", type=["jpg","png"])
        if up_img: st.image(up_img, use_column_width=True)
    with c_txt:
        msg = st.text_area("Mensagem", "Olá! 🦐\nHoje tem caruru fresquinho!", height=150)
    ...
    if df_c.empty: st.warning("Sem clientes.")
        ...
        st.data_editor(
            df_show,
            column_config={
                "Link": st.column_config.LinkColumn("Enviar", display_text="🚀 Zap"),
    ...
    with t1:
        with st.form("cli_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Salvar"):
                ...
                if cliente_existe(n, df_c):
                    st.warning(f"Cliente '{n}' já existe!")
                elif not n:
                    st.error("Nome obrigatório")
                else:
                    z_safe = limpar_telefone(z)
                    novo = pd.DataFrame([{"Nome": n, "Contato": z_safe, "Observacoes": o}])
                    st.session_state.clientes = pd.concat([df_c, novo], ignore_index=True)
        ...
        if st.button("📄 Exportar Lista PDF"):
            ...
            if pdf: st.download_button("Baixar Lista", pdf, "Clientes.pdf", "application/pdf")
    ...
    st.title("🛠️ Admin")
    t1, t2 = st.tabs(["Logs", "Histórico"])
    with t1:
        if os.path.exists(ARQUIVO_LOG):
            with open(ARQUIVO_LOG, "r") as f: log = f.read()
            st.text_area("", log, height=200)
            if st.button("Limpar Logs"): open(ARQUIVO_LOG, 'w').close(); st.rerun()
    with t2:
        if os.path.exists(ARQUIVO_HISTORICO):
            Hist = pd.read_csv(ARQUIVO_HISTORICO).sort_values("Timestamp", ascending=False)
            st.dataframe(Hist, use_container_width=True)
            if st.button("Limpar Histórico"): os.remove(ARQUIVO_HISTORICO); st.rerun()
