...
import zipfile
import logging
import re # Nova biblioteca para limpar telefone
...
# ==============================================================================
# FUNÇÕES AUXILIARES (VALIDAÇÃO E LIMPEZA)
# ==============================================================================
def limpar_hora_rigoroso(h):
    """Converte horários variados para objeto time"""
    try:
...
def limpar_telefone(telefone):
    ...
    # Usa Regex para deixar apenas digitos
...
def gerar_id_sequencial(df):
    try:
        ...
        # Garante int
        col = pd.to_numeric(df['ID_Pedido'], errors='coerce').fillna(0).astype(int)
...
def calcular_total(caruru, bobo, desconto):
    try:
        ...
        d = max(0.0, min(100.0, float(desconto or 0))) # Trava desconto entre 0 e 100
...
# ==============================================================================
# BANCO DE DADOS (CARREGAMENTO E SALVAMENTO)
# ==============================================================================
...
def carregar_pedidos():
    ...
    try:
        ...
        # Normalização de Texto e Status
...
# --- FUNÇÃO QUE FALTAVA NA VERSÃO ANTERIOR ---
...
with st.sidebar:
    ...
    st.divider()
    menu = st.radio("Navegação", ["Dashboard do Dia", "Novo Pedido", "Gerenciar Tudo", "🖨️ Relatórios & Recibos", "👥 Cadastrar Clientes", "🛠️ Manutenção"])
    st.divider()
...
if menu == "Dashboard do Dia":
        ...
        if not df_dia.empty:
            ...
            if not edited.equals(df_dia):
                ...
                for idx in edited.index:
                    ...
                    if mask.any():
                        df_glob.loc[mask, edited.columns] = edited.loc[idx, :].values
    ...
    with st.form("form_novo", clear_on_submit=True):
        ...
        if st.form_submit_button("💾 SALVAR"):
            # Validações Blindadas
            erro = False
            if not c_sel:
                st.error("❌ Selecione um cliente.")
                erro = True
            if qc <= 0 and qb <= 0:
                st.error("❌ O pedido precisa ter pelo menos 1 Caruru ou 1 Bobó.")
                erro = True
            if qc < 0 or qb < 0:
            ...
            if not erro:
                try:
                    ...
                    st.success(f"Pedido #{nid} criado com sucesso!")
                    st.rerun()
                except Exception as e:
        ...
        try:
            ...
            if sel:
                ...
                if t and len(t) >= 10:
                    ...
                    st.warning("Telefone inválido para Zap.")
        except: pass
        ...
        with st.form("cli_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Salvar"):
                ...
                if not st.session_state.clientes.empty:
                    if n.lower().strip() in st.session_state.clientes['Nome'].str.lower().str.strip().values:
                        existe = True
                
                if not n: st.error("Nome obrigatório.")
                elif existe: st.warning(f"O cliente {n} já existe!")
                else:
                    z_limpo = limpar_telefone(z)
                    novo = pd.DataFrame([{"Nome": n, "Contato": z_limpo, "Observacoes": o}])
